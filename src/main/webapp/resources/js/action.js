...
function loadRefData() {
	$.post(urlHolder.refdataUserProject, function(response) {
		if (response) {
			...
			for ( var i = 0; i < response.project.length; i++) {
				listItems += "<option value='" + response.project[i].projectId
						+ "'>" + response.project[i].projectDesc + "</option>";
			}
			$("#projectOption").html(listItems);
			loadTaskDetails();
			}else{
				alert("Please contact admin to add the Project");
			}
		}
	});
}
...
function webtimeWindow(){
	window.location.href = "/todo/jsp/webtime.jsp";
}

function fillWebProjectForm() {
	var selected = $('input:radio[name=index]:checked').val();
	$('#editwebprojName').val($('#webtimeProjects').data('model')[selected].projectName);
	oldtxt = $('#editwebprojName').val();
}

function loadWebProjectTable() {
	$.post(urlHolder.webprojRecords, function(response) {
		$('#webtimeProjects').find('tbody').remove();
		if (response) {
			for ( var i = 0; i < response.webProjects.length; i++) {
				var row = '<tr>';
				row += '<td><input type="radio" name="index" id="index" value="'
					+ i + '"></td>';
				row += '<td>' + response.webProjects[i].projectName + '</td>';
				row += '</tr>';
				$('#webtimeProjects').append(row);
			}
				$('#webtimeProjects').data('model', response.webProjects);
		}
	});
}

function submitAddWebProjRecord() {
	$.post(urlHolder.addwebproj, {
		projectName : $('#webprojName').val()
	}, function(response) {
		if (response != null) {
...
function submitEditWebProjRecord(){
	$.post(urlHolder.updatewebproj, {
		projectName : $('#editwebprojName').val(),
		oldValue : oldtxt
	}, function(response) {
		if (response != null) {
...
>>>>>>> Added for TimeSheet enhancement.
